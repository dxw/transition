...
describe SitesController do
  ...
  describe '#create' do
    it 'cannot assign to an organisation user does not have permission for'
  end
    ...
    describe '#create' do
      ...
      it 'creates a new site' do
        params = {
          ...
          launch_date: Date.today + 10.days,
          abbr: 'MOJ',
        ...
        expect(site.global_redirect_append_path).to eq(params[:global_redirect_append_path])
        expect(site.homepage_title).to eq(params[:homepage_title])
        expect(site.organisation).to eq(organisation)
      end

      it 'creates nested hosts'
    end
